...
function findStartingPoints(grid,word){
    ...
    for( var gridRow = 0; gridRow < grid.length; gridRow++ ){
        for( var gridCol = 0; gridCol < grid[0].length; gridCol++ ){
...
function DFS(grid, row, col, visited, word, letter, found){
        ...
        for( var offset_pairs = 0; offset_pairs < 8; offset_pairs++ ){ // Checking all the neighbours
            var x = offsets[offset_pairs][0] + row 
            var y = offsets[offset_pairs][1] + col
...
function findAllSolutions(grid, dictionary){
    ...
    for ( var i = 0; i < dictionary.length; i++ ){ //iterating thru dictionary
        
        var word = dictionary[i]; //extracting words
        if(!isWordValid(word)){
            continue
        }
        

        var starting_points = findStartingPoints(grid,word); // finding the position index pair(s) for first letter of word

        if(starting_points === []){ //if no starting_points, move on to the next word
            continue
        }

        for ( var pair = 0; pair < starting_points.length; pair++ ){ // iterating the various starting_points
            //creating a new visited arr for each time we check a diff start pt
            let visited = new Array(grid.length)
            for(var visit_row = 0; visit_row < grid.length; visit_row++){
                visited[visit_row] = new Array(grid[0].length)
                for(var visit_col = 0; visit_col < grid[0].length; visit_col++){
                    visited[visit_row][visit_col] = 0;
                }
            }
            
            var found = false // init found to false for each point we check for the word

            DFS(grid, starting_points[pair][0], starting_points[pair][1], visited, word, 1, found) // Diving in
